...
# TP=2 — two H200 (GPUs 4,5; both NUMA 2)
#
...
#  - 16 attention heads, 2 KV heads (GQA 8:1) → TP=2 splits attn into 8 heads/rank ✓
#  - 16 linear_num_key_heads, 32 linear_num_value_heads → TP=2 must divide both ✓ (8 and 16)
#  - max_position_embeddings 262144, BF16 weights (~70 GB → ~35 GB/rank)
#
# Notes:
#  - No --kv-cache-dtype fp8_e4m3: BF16 model, native KV.
#  - --linear-attn-backend triton: required for the 30 linear-attention layers.
#  - --attention-backend fa3: default for the 10 full-attention layers.
#  - SGLANG_MM_FEATURE_CACHE_MB=2048 — same as 32B TP=2 to share image features across TP ranks.
#  - MAX_RUNNING_REQUESTS lowered vs TP=1 (mirrors 32B convention) to give KV cache headroom.
...
export RESULT_BASE=/hongming/res20_agg_h200_35b/tp2

# Cache image features in shared memory across TP ranks (avoids per-rank duplication)
...
echo "=========================================="
echo "Dynamo SGLang Aggregated EPD Server — TP=2"
...
echo "  - MM feature cache: ${SGLANG_MM_FEATURE_CACHE_MB} MB"
...
echo "This may take ~5-7 min for model loading + NCCL init (35B BF16 MoE, TP=2)..."
