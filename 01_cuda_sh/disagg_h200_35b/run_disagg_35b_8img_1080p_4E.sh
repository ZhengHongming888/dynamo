#!/bin/bash
# Disagg 35B 4E sweep — 8img/1080p only, all 7 rates, np=32.
# Derived from run_disagg_35b_sweep.sh; same client knobs, different output
# path so we don't clobber the 1E baseline at /hongming/res22_disagg_h200_35b_sweep/8img_1080p/.
#
# Topology assumption: PD on giga01 H200 (started via start_sglang_pd_cuda_35b_giga01.sh),
# 4 encoders on B70. Verify with:
#   no_proxy='*' ETCDCTL_API=3 etcdctl --endpoints=http://172.26.46.75:12379 \
#       get --prefix v1/instances/ --keys-only

set -u

MODEL=/mnt/weka/data/llm-d-models-pv/models--Qwen--Qwen3.5-35B-A3B/snapshots/59d61f3ce65a6d9863b86d2e96597125219dc754
BASE_URL=http://localhost:7001
RESULT_BASE=/hongming/res22_disagg_h200_35b_sweep/8img_1080p_4E
...
run_one () {
  ...
  local logfile="$LOG_DIR/bench_disagg_35b_4E_8img_1080p_r${rate}_np${NP}.log"
  mkdir -p "$outdir"
  if is_done "$outfile"; then
    echo "[$(date -u +%H:%M:%S)] SKIP rate=$rate np=$NP — already done"
    return 0
  fi
  echo "[$(date -u +%H:%M:%S)] >>> 4E disagg 35B 8img/1080p rate=$rate np=$NP"
...
echo "==== 4E disagg 35B 8img/1080p sweep starting at $(date -u) ===="
...
echo "==== 4E disagg 35B 8img/1080p sweep done at $(date -u) ===="
