#!/bin/bash
# Disagg 35B sweep — 8img/1080p, all 7 rates, np=32.
# Topology: PATCHED giga01 H200 PD (h200_cuda_nixl.patch active) + 4 B70 XPU encoders.
# PD log: pd_worker_giga01_h200_patched_debug_*.log (DYN_LOG=debug)

set -u

MODEL=/mnt/weka/data/llm-d-models-pv/models--Qwen--Qwen3.5-35B-A3B/snapshots/59d61f3ce65a6d9863b86d2e96597125219dc754
BASE_URL=http://localhost:7001
RESULT_BASE=/hongming/res22_disagg_h200_35b_sweep/8img_1080p_h200_patched_b70_4E
...
run_one () {
  ...
  local logfile="$LOG_DIR/bench_disagg_35b_h200_patched_b70_4E_8img_1080p_r${rate}_np${NP}.log"
  mkdir -p "$outdir"
  if is_done "$outfile"; then
    echo "[$(date -u +%H:%M:%S)] SKIP rate=$rate np=$NP — already done"
    return 0
  fi
  echo "[$(date -u +%H:%M:%S)] >>> patched + B70_4E disagg 35B 8img/1080p rate=$rate np=$NP"
  ...
  while [ $attempt -lt $MAX_WARMUP_RETRIES ]; do
    ...
    if grep -q "Warmup failed\|UR_RESULT_ERROR_OUT_OF_DEVICE_MEMORY\|level_zero\|CUDA out of memory" "$logfile" 2>/dev/null; then
...
echo "==== patched + B70_4E disagg 35B 8img/1080p sweep starting at $(date -u) ===="
...
echo "==== patched + B70_4E disagg 35B 8img/1080p sweep done at $(date -u) ===="
