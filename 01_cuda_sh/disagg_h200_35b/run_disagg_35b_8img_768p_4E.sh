#!/bin/bash
# Disagg 35B 4E sweep — 8img/768p only, all 7 rates, np=32.
# Companion to run_disagg_35b_4img_768p_4E.sh and run_disagg_35b_8img_1080p_4E.sh.

set -u

MODEL=/mnt/weka/data/llm-d-models-pv/models--Qwen--Qwen3.5-35B-A3B/snapshots/59d61f3ce65a6d9863b86d2e96597125219dc754
BASE_URL=http://localhost:7001
RESULT_BASE=/hongming/res22_disagg_h200_35b_sweep/8img_768p_4E
...
run_one () {
  ...
  local logfile="$LOG_DIR/bench_disagg_35b_4E_8img_768p_r${rate}_np${NP}.log"
  mkdir -p "$outdir"
  if is_done "$outfile"; then
    echo "[$(date -u +%H:%M:%S)] SKIP rate=$rate np=$NP — already done"
    return 0
  fi
  echo "[$(date -u +%H:%M:%S)] >>> 4E disagg 35B 8img/768p rate=$rate np=$NP"
...
echo "==== 4E disagg 35B 8img/768p sweep starting at $(date -u) ===="
...
echo "==== 4E disagg 35B 8img/768p sweep done at $(date -u) ===="
