#!/bin/bash
# Disagg 35B 5E HYBRID sweep — 8img/1080p only, 7 rates, np=32.
# Topology: PD on giga01 H200 + 1×dell06 H200 encoder + 4×B70 XPU encoders.
# Round-robin dispatcher will mix requests between fast (dell06) and slow (B70) encoders.

set -u

MODEL=/mnt/weka/data/llm-d-models-pv/models--Qwen--Qwen3.5-35B-A3B/snapshots/59d61f3ce65a6d9863b86d2e96597125219dc754
BASE_URL=http://localhost:7001
RESULT_BASE=/hongming/res22_disagg_h200_35b_sweep/8img_1080p_5E_hybrid
...
run_one () {
  ...
  local logfile="$LOG_DIR/bench_disagg_35b_5E_hybrid_8img_1080p_r${rate}_np${NP}.log"
  mkdir -p "$outdir"
  if is_done "$outfile"; then
    echo "[$(date -u +%H:%M:%S)] SKIP rate=$rate np=$NP — already done"
    return 0
  fi
  echo "[$(date -u +%H:%M:%S)] >>> 5E hybrid disagg 35B 8img/1080p rate=$rate np=$NP"
...
echo "==== 5E hybrid disagg 35B 8img/1080p sweep starting at $(date -u) ===="
...
echo "==== 5E hybrid disagg 35B 8img/1080p sweep done at $(date -u) ===="
