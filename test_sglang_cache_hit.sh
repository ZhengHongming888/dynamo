#!/bin/bash
# Cache-hit test for SGLang benchmark - 32B FP8 model with identical images.
# Uses --image-content blank so every image is byte-identical, exercising
# --enable-mm-global-cache on the worker side.
# Usage: ./test_sglang_cache_hit.sh [rates...]
# Example: ./test_sglang_cache_hit.sh 1.0
#          ./test_sglang_cache_hit.sh        # uses default rates
...
IMAGE_CONTENT=blank        # <-- key knob: identical images so cache hits after first request
#IMAGE_RESOLUTION="1024x768"

# Default rate for cache-hit comparison: matches the prefill-heavy disagg vs agg test
DEFAULT_RATES=(1.0)
...
RESULT_BASE="${RESULT_BASE:-/hongming/res3/h200_agg_tp2_32b_image8_1080p_np_rates}"
