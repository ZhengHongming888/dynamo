#!/bin/bash
# Run multiple rate tests for SGLang benchmark - Qwen3.5-35B-A3B (BF16 MoE) with 8 images at 1080p
# Mirrors test_sglang_mult_rates_32b_1080p_np64_over_rates.sh — only model + result paths changed.
# Usage: ./test_sglang_mult_rates_35b_1080p_np64_over_rates.sh [rates...]
# Example: ./test_sglang_mult_rates_35b_1080p_np64_over_rates.sh 0.1 0.2 0.3
#          ./test_sglang_mult_rates_35b_1080p_np64_over_rates.sh   # uses default rates
...
# Default rates for 35B model with 8 images at 1080p
# Fixed num_prompts=64 for ALL rates (apples-to-apples sweep)
DEFAULT_RATES=(0.25 0.5 1.0 1.5 2.0 3.0)
...
# Default points to res20_agg_h200_35b (matches start scripts).
RESULT_BASE="${RESULT_BASE:-/hongming/res20_agg_h200_35b/h200_agg_35b_image8_1080p_np_rates}"
TEST_DIR="${RESULT_BASE}/test_sglang_multi_rates_1080p_${TIMESTAMP}"
mkdir -p "$TEST_DIR"

echo "=========================================="
echo "SGLang Multi-Rate Benchmark - 35B BF16 MoE (1080p)"
echo "=========================================="
echo "Model:       $MODEL"
echo "Port:        $PORT"
...
echo ""
echo "=========================================="
echo "BENCHMARK SUMMARY - ALL RATES (35B BF16 MoE 1080p)"
echo "=========================================="
echo ""
...
# Also save the table output to a file
{
    echo "BENCHMARK SUMMARY - ALL RATES (35B BF16 MoE 1080p)"
    echo "Generated: $(date)"
    echo "Configuration: 8 images at $IMAGE_RESOLUTION, np=$FIXED_NUM_PROMPTS for all rates"
    echo ""
    echo "=== Request Throughput & Latency ==="
    printf "%-12s %-12s %-12s %-12s %-12s %-12s %-12s\n" \
        "Target_RPS" "Actual_RPS" "Success" "Mean_E2E" "Median_E2E" "P99_E2E" "Concurrency"
