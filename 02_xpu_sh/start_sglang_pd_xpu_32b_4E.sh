...
# This starts: 4 Encode workers on Intel XPU devices for multimodal processing
...
# XPU Devices - Choose 4 different devices from: 0, 1, 2, 3, 4, 5, 6, 7
export XPU_DEVICE_1=2  # First Intel XPU device ID
export XPU_DEVICE_2=4  # Second Intel XPU device ID
export XPU_DEVICE_3=6  # Third Intel XPU device ID
export XPU_DEVICE_4=0  # Fourth Intel XPU device ID
...
export SIDE_CHANNEL_PORT_3=22100  # NIXL side channel port for encoder 3
export SIDE_CHANNEL_PORT_4=22101  # NIXL side channel port for encoder 4
...
export UCX_NIC_3=mlx5_1:1         # InfiniBand NIC for encoder 3 (shared)
export UCX_NIC_4=mlx5_2:1         # InfiniBand NIC for encoder 4 (shared)
...
# Wait before starting second worker
echo ""
...
# Wait before starting third worker
echo ""
echo "Waiting 10 seconds before starting third encoder..."
sleep 10

# Start Intel XPU Encode Worker 3
echo ""
...
python3 -m dynamo.sglang \
    ...
    2>&1 | tee -a "$LOG_DIR/encode_xpu_32b_3.log" &

ENCODE_PID_3=$!
echo "Encode Worker 3 started with PID: $ENCODE_PID_3"

# Wait before starting fourth worker
echo ""
echo "Waiting 10 seconds before starting fourth encoder..."
sleep 10

# Start Intel XPU Encode Worker 4
echo ""
...
python3 -m dynamo.sglang \
    ...
    2>&1 | tee -a "$LOG_DIR/encode_xpu_32b_4.log" &
...
echo "  - Log: $LOG_DIR/encode_xpu_32b_3.log"
echo ""
echo "Encoder 4 (32B FP8):"
...
echo "  • Total XPU memory: ~8-16GB for four encoders"
echo "  • Load time: ~5-10 minutes per encoder (~40-50 min total with delays)"
echo "  • Same model as CUDA decode worker for consistency"
echo ""
echo "=========================================="
echo ""
echo "To monitor encode workers:"
echo "  tail -f $LOG_DIR/encode_xpu_32b_{1,2,3,4}.log"
echo ""
echo "To check when ready:"
echo "  grep -i 'registered\\|ready\\|succeeded' $LOG_DIR/encode_xpu_32b_{1,2,3,4}.log"
echo ""
...
echo "  Monitor all four encoders to ensure balanced load distribution"
echo "  Each encoder should process ~25% of incoming multimodal requests"
echo ""
