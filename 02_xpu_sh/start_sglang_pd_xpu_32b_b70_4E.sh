...
# Pair : H200 @ 172.26.46.75 (sc09super21-h200) - matches start_sglang_pd_xpu_32b_b70.sh
#
...
#
# Encoder-only mode: each worker loads ONLY the vision tower (~1.6 GiB on XPU),
# NOT the 33 GiB language model -- that fits in the 32 GiB B70 VRAM.
...
# Same scheme as start_sglang_pd_xpu_32b_4E.sh.
...
launch_encoder() {
    ...
    local LOG="$LOG_DIR/encode_xpu_32b_b70_${N}.log"
...
echo ""
echo "Logs:    $LOG_DIR/encode_xpu_32b_b70_{1,2,3,4}.log"
echo "Tail:    tail -f $LOG_DIR/encode_xpu_32b_b70_{1,2,3,4}.log"
echo "Ready:   grep -i 'registered\\|ready\\|succeeded' $LOG_DIR/encode_xpu_32b_b70_{1,2,3,4}.log"
echo "Stop:    kill $ENCODE_PID_1 $ENCODE_PID_2 $ENCODE_PID_3 $ENCODE_PID_4"
echo "         # or: pkill -f 'dynamo.sglang.*multimodal-encode-worker'"
echo ""
echo "Notes:"
echo "  - --encoder-only loads ONLY vision tower (~1.6 GiB), NOT 33 GiB LLM weights."
