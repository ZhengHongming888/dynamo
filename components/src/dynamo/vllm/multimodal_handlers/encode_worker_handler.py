...
class EncodeWorkerHandler:
    ...
    def log_registration(self, endpoint, namespace: str, component: str, endpoint_name: str):
                    ...
                    for img_idx, single_image in enumerate(loaded_images):
                        with _nvtx.annotate(
                            ...
                            image_embeds = await asyncio.to_thread(
                                self.image_processor, images=[single_image], return_tensors="pt"
                            )

                        with _nvtx.annotate(f"mm:enc:vision_encode_{img_idx}", color="red"):
                    ...
                    if is_qwen_vl_model(self.model):
                        ...
                        if expected_total != actual_total:
                            ...
                            if len(sizes) > 1 and actual_total == sizes[0]:
                                logger.info("Using embeddings as-is (already per-image)")
