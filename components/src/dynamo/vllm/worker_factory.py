...
class WorkerFactory:
    ...
    async def _create_multimodal_worker(
        ...
        encode_worker_client, encoder_device_mapping = await self._maybe_get_encode_worker_client(
            runtime, config
        )
    ...
    async def _create_decode_worker(
        ...
        # performed
        encode_worker_client, _ = await self._maybe_get_encode_worker_client(
            runtime, config
        )
    ...
    async def _maybe_get_encode_worker_client(
        ...
        if config.route_to_encoder:
            ...
            logger.info(f"Found {len(device_info_instances)} device_info endpoint instances")

            for device_info_instance_id in device_info_instances:
                try:
                    logger.info(f"Querying device_info instance {device_info_instance_id}...")
                    response_stream = await device_info_client.direct("", device_info_instance_id)
                    async for response in response_stream:
                        ...
                        if generate_instance_id is not None:
                            ...
                            logger.error(f"device_info response missing generate_instance_id: {device_info}")
                        break  # Only need first response
                except Exception as e:
                    logger.error(f"Failed to query device_info instance {device_info_instance_id}: {e}")
